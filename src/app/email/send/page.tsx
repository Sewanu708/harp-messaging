...
function Email() {
    ...
    return (
        <section className="py-4 px-2 sm:px-4 w-full relative h-screen overflow-y-auto">
            {isAddDomainOpen && <AddDomainPopup setSelectedDomain={setSelectedDomain} onClose={toggleAddDomainPopup} />}
            <div className="text-xl sm:text-2xl py-2 w-full bg-white fixed font-semibold text-[#0F6C68] z-10">Send Email</div>
            <div className="mt-16 mb-4 p-2 sm:p-4 bg-zinc-100 rounded-md w-full">
                <div className="mb-4 flex flex-col items-end justify-end gap-2">
                    {selectedDomain.length > 0 ? (
                        ...
                        <>
                            <Button variant={"outline"} onClick={toggleAddDomainPopup} className="ml-0 sm:ml-2 cursor-pointer hover:bg-zinc-100 transition-all duration-300 ease-in-out w-full sm:w-auto">
